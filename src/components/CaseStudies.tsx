          ...
          return (
            ...
            >
              <span className="absolute pointer-events-none select-none"
                style={{ fontFamily: "'DM Serif Display', serif", fontSize: 130, fontStyle: "italic", fontWeight: 400, lineHeight: 1, bottom: -16, right: -8, color: style.watermark, transform: isActive ? "scale(1.08) translate(-4px,-8px)" : "none", transition: "transform 0.5s cubic-bezier(0.16,1,0.3,1)", opacity: isActive ? 0.5 : 1 }}>
                {cs.id}
              </span>
              <div className="flex flex-col" style={{ gap: 6 }}>
                ...
                <h3 className="font-normal"
                  style={{ fontFamily: "'DM Serif Display', serif", fontSize: isActive ? 23 : 17, lineHeight: 1.2, color: style.title, transition: "font-size 0.4s cubic-bezier(0.16,1,0.3,1)" }}
                  dangerouslySetInnerHTML={{ __html: cs.title.replace(/—(.+)$/, `— <em style="font-style:italic;color:${style.accent}">$1</em>`) }}
                />
                <p style={{ fontSize: 13, lineHeight: 1.7, color: style.desc, opacity: isActive ? 1 : 0, maxHeight: isActive ? 80 : 0, overflow: "hidden", marginTop: isActive ? 8 : 0, transition: "opacity 0.3s 0.15s, max-height 0.4s, margin-top 0.3s" }}>
                  {cs.description}
                </p>
              </div>
              <div className="relative flex flex-col" style={{ gap: 4, zIndex: 2 }}>
                <div style={{ fontFamily: "'DM Serif Display', serif", fontSize: 30, fontWeight: 400, lineHeight: 1, color: style.title }}>
                  {cs.metric}<em style={{ fontStyle: "normal", color: style.accent, fontSize: 18 }}> {cs.metricLabel.split(" ")[0]}</em>
                </div>
                ...
                <div className="flex flex-wrap" style={{ gap: 5, opacity: isActive ? 1 : 0, maxHeight: isActive ? 60 : 0, overflow: "hidden", marginTop: isActive ? 10 : 0, transition: "opacity 0.3s 0.2s, max-height 0.4s, margin-top 0.3s" }}>
                  {cs.outcomes.slice(0, 3).map((o) => (
                    <span key={o.label} style={{ fontSize: 10, fontWeight: 600, padding: "4px 10px", borderRadius: 100, background: style.pillBg, color: style.pillColor, border: `1px solid ${style.pillBorder}`, whiteSpace: "nowrap" }}>
                      {o.metric} {o.label}
                    </span>
                  ))}
                </div>
                <div className="inline-flex items-center" style={{ gap: 8, fontSize: 12, fontWeight: 600, color: style.accent, opacity: isActive ? 1 : 0, marginTop: isActive ? 12 : 0, transition: "opacity 0.3s 0.25s, margin-top 0.3s", width: "fit-content" }}>
      ...
      >
        {caseStudies.map((cs, i) => {
          ...
          return (
            ...
            >
              <span className="absolute pointer-events-none select-none"
                style={{ fontFamily: "'DM Serif Display', serif", fontSize: 80, fontStyle: "italic", bottom: -8, right: -4, color: style.watermark, lineHeight: 1, opacity: 0.6 }}>
                {cs.id}
              </span>
              <div className="relative z-10">
                ...
                <h3 className="font-normal mb-2"
                  ...
                  dangerouslySetInnerHTML={{ __html: cs.title.replace(/—(.+)$/, `— <em style="font-style:italic;color:${style.accent}">$1</em>`) }}
                />
                <p style={{ fontSize: 13, lineHeight: 1.7, color: style.desc, marginBottom: 16 }}>{cs.description}</p>
                <div style={{ fontFamily: "'DM Serif Display', serif", fontSize: 26, fontWeight: 400, lineHeight: 1, color: style.title, marginBottom: 4 }}>
                  {cs.metric}<em style={{ fontStyle: "normal", color: style.accent, fontSize: 16 }}> {cs.metricLabel.split(" ")[0]}</em>
                </div>
                <div style={{ fontSize: 11, color: style.lbl, marginBottom: 14 }}>{cs.metricLabel.split(" ").slice(1).join(" ")}</div>
                <div className="flex flex-wrap" style={{ gap: 5, marginBottom: 14 }}>
                  {cs.outcomes.slice(0, 3).map((o) => (
                    <span key={o.label} style={{ fontSize: 10, fontWeight: 600, padding: "4px 10px", borderRadius: 100, background: style.pillBg, color: style.pillColor, border: `1px solid ${style.pillBorder}` }}>
                      {o.metric} {o.label}
                    </span>
                  ))}
                </div>
                <div className="inline-flex items-center gap-2" style={{ fontSize: 12, fontWeight: 600, color: style.accent }}>
