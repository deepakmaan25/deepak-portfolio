...
const About = () => {
  ...
  return (
    <section id="about">
      ...
      <div className="py-24 md:py-[96px]">
        <div className="border-t border-border" />
        <div ref={ref} className="px-6 max-w-site mx-auto pt-24 md:pt-[96px]" style={{ opacity: visible ? 1 : 0, transform: visible ? 'translateY(0)' : 'translateY(20px)', transition: 'opacity 500ms ease, transform 500ms ease' }}>
          <div className="grid lg:grid-cols-2 gap-16 lg:gap-24">
            ...
            <div className="flex items-center justify-center">
              <div className="relative">
                ...
                {/* Credential badge */}
                <div className="absolute -bottom-4 -right-5 z-20 bg-background border border-border rounded-xl px-4 py-2.5" style={{ boxShadow: '0 4px 16px rgba(0,0,0,0.08)' }}>
      ...
      {/* SUBSECTION 2 — Skills Strip (dark) */}
      <div className="py-8" style={{ background: 'hsl(var(--skills-strip-bg))' }}>
        <div className="px-6 max-w-site mx-auto flex flex-col md:flex-row md:items-start gap-6 md:gap-0">
          {skillGroups.map((group, gi) =>
          <div key={group.label} className="flex-1 flex flex-col gap-3" style={{ borderLeft: gi > 0 ? '1px solid hsl(var(--skills-strip-divider))' : undefined, paddingLeft: gi > 0 ? 24 : 0 }}>
              <span className="text-[10px] uppercase tracking-[0.15em] font-semibold" style={{ color: 'hsl(var(--skills-strip-label))' }}>{group.label}</span>
              ...
              <span
                ...
                onMouseLeave={(e) => {e.currentTarget.style.borderColor = 'hsl(var(--skills-strip-tag-border))';e.currentTarget.style.color = 'hsl(var(--skills-strip-text))';}}>
      ...
      {/* SUBSECTION 3 — Experience Timeline */}
