    ...
    <div style={{
      ...
      {stats.map((s, i) => (
        <div key={i} style={{
          ...
          {/* Subtle watermark */}
          <div style={{ position: "absolute", bottom: -8, right: 8, fontFamily: FD, fontSize: 60, fontWeight: 800, color: "rgba(99,102,241,0.04)", lineHeight: 1, userSelect: "none", pointerEvents: "none" }}>{s.value.replace(/[^0-9%x+]/gi, "")}</div>
      ...
      {quotes.map((q, i) => (
        <blockquote key={i} style={{
          ...
          <span style={{
            ...
            color: "rgba(99,102,241,0.2)", lineHeight: 1,
...
// ── Section wrapper — all full width now ─────────────────────────────────────
    ...
    <div style={{ minHeight: "100vh", background: "hsl(var(--background))", paddingTop: 64 }}>
      ...
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, ease: [0.16, 1, 0.3, 1] }}>
        <div style={{ ...innerStyle, paddingTop: "clamp(32px,5vw,80px)", paddingBottom: "clamp(24px,4vw,52px)" }}>
          ...
          {/* Tagline */}
          <p style={{ fontFamily: F, fontSize: "clamp(15px, 1.8vw, 18px)", lineHeight: 1.7, color: "hsl(var(--muted-foreground))", maxWidth: 640, marginBottom: 44, fontWeight: 400 }}>
            {cs.tagline}
          </p>

          {/* Meta grid */}
          <div style={{ borderTop: "1px solid hsl(var(--border))", paddingTop: 28, display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "0" }}>
            {[
