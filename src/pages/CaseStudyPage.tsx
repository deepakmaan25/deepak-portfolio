...
const CASE_STUDIES: Record<string, CaseStudy> = {
  ...
  'buzztro': {
    ...
    content: [
      {
        id: 'overview', heading: 'Overview',
        body: `Buzztro's model is collective buying -shoppers commit together and the price drops as more people join. Each product runs as a "drop": it opens, fills as buyers pre-book, and completes once enough demand lands. Simple on paper. The design problem was making a conditional, community-driven pricing model feel trustworthy and rewarding rather than confusing or pressuring.\n\nSolo designer on a 2-month contract, direct to founder. Competitor research, information architecture, wireframes, and 40+ high-fidelity screens across 5 core flows. Shipped to production.`,
        highlight: null, quote: null, image: null,
      },
      {
        id: 'research', heading: 'Research',
        body: `Mapped how group-buying platforms handle the collective-pricing mechanic across markets -Pinduoduo, Meesho, DealShare, Groupon -looking at which moments they explained, which they hid, and where the experience broke down.\n\n**The price doesn't match the label.** Most platforms show a discounted price on the card, but it's conditional. When users noticed this at checkout, trust collapsed. The fix had to happen upstream.\n\n**Drop status is buried.** How many joined? How many more needed? These questions decide whether a user commits -and were routinely hidden until after the user had already committed to buy.\n\n**Post-purchase is an afterthought.** Waiting is the product for collective buying -that's where the shopper lives for hours after payment. It needed its own design, not a generic confirmation screen.`,
      ...
      },
      {
        ...
        body: `The card had to carry three things a normal card doesn't: how far the drop has progressed, the price it unlocks, and proximity to the next tier -without becoming cluttered.\n\nThe version that shipped places a progress bar beneath the product title -current price on one side, target price on the other, "X joined, Y more to unlock" completing the picture. A status badge drives the whole card: Opening Soon, Live Now, Booked, Drop Complete -each with its own contextual CTA (Start the Drop, Pre Book Now, Notify Me, Buy now). Cards became taller than standard. That's the right trade-off. If the drop progress isn't readable at a glance, it's not Buzztro anymore.`,
        highlight: null, quote: null, image: '/buzztro/card-system.png',
      },
      {
        id: 'sol-pdp', heading: 'Solution 2: Product detail page',
        body: `Standard PDP hierarchy: image, title, price, CTA. On Buzztro, price is the live output of an active drop -which required inverting that hierarchy entirely.\n\nThe circular gauge sits directly under the title -a live readout of how full the drop is, with the discounted target alongside. One glance and the mechanic is understood without reading any copy. A dual CTA gives users both paths: buy now at the current price, or join the price drop for the lower one. Below the fold: familiar e-commerce patterns. Novelty only where it earns its place.`,
        highlight: null, quote: null, image: '/buzztro/pdp-overview.png',
      },
      {
        id: 'sol-checkout', heading: 'Solution 3: Checkout',
        body: `Buzztro splits the commitment in two, and the checkout had to make that legible. You don't pay the full amount upfront on a maybe. You pay a small booking amount to reserve your spot in the drop, and only complete the full purchase once the drop actually succeeds.\n\nThe flow makes those two phases explicit: a stepper that reads Join the drop → Drop succeeds → Checkout, so users always know which stage they're in. The final checkout is a familiar multi-step structure -address, payment, review -but the order summary carries the drop-specific math: the drop discount, the booking amount already paid, and the final total. The most-reviewed copy in the project was the reassurance layer: what the booking buys you, and what happens if the drop doesn't complete. If those sentences were confusing, the trust story collapsed.`,
        highlight: null, quote: null,
        images: ['/buzztro/booking-checkout-mobile.png', '/buzztro/booking-checkout-desktop.png'],
      },
      {
        id: 'sol-cart', heading: 'Solution 4: Cart',
        body: `Buzztro's cart is closer to a waiting room than a holding area -items the user has committed to, each with its own live drop state and potential final price.\n\nPer-item drop status instead of a flat line-item list. Totals show two numbers: what you pay at current drop levels, and what you'd pay if every drop completes. The reassurance about what happens to an incomplete drop appears here too, before checkout -setting expectations early rather than surprising users in the final step.`,
        highlight: null, quote: null, image: '/buzztro/checkout-address.png',
      },
      {
        id: 'sol-post', heading: 'Solution 5: Post-purchase',
        body: `Between booking and shipment, a Buzztro customer lives in a waiting state that can last hours or days. If it feels empty, users assume something broke. This is the one surface where the interface leans on "pool" language -the waiting screen is about the community filling up, and the copy on screen reflects that.\n\nThe order status screen shows live pool progress, countdown to close, current price band, and a share CTA. That share action isn't a growth mechanic bolted on -it's the most useful thing the user can actually do in that moment: filling the pool faster is what gets them their lower price. Two additional states designed in full: pool filled and pool failed. Both are full screens. Neither is a toast.`,
        highlight: null, quote: null,
        images: ['/buzztro/order-waiting.png', '/buzztro/order-details.png'],
      },
      {
        id: 'reflection', heading: 'Reflection',
        body: `Making drop progress the primary signal was the thesis of the entire product -a circular gauge on the detail screens, a progress bar on the cards, the same idea everywhere a user looks. Once I committed to that, every other decision got easier -hierarchy, copy, layout all resolved against it. A single strong principle does that.\n\nThe post-purchase state nearly became an afterthought. Sitting with the flow as a user changed that. For a platform where purchase is conditional, the hours after booking are where the actual experience lives.\n\n**Outcome:** The product worked. Users understood the drop mechanic without reading copy. The gauge did the explaining. The company eventually paused on funding -a market reality, not a product one.`,
        highlight: null, quote: null, image: null,
      },
    ],
  },
